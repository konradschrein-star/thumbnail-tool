...
export async function callNanoBanana(
  ...
  try {
    ...
    try {
      ...
      if (isUnavailable) {
        console.warn('   ⚠️ Nano Banana is unavailable due to high demand. Retrying taking a backup model (nano-banana-flash-preview) so we can at least test our generation engine...');
        try {
          response = await callWithPayload(primaryContent, 'nano-banana-flash-preview');
          ...
          if (backupMsg.includes("Unable to process input image") && imageParts.length > 1) {
            ...
            response = await callWithPayload(fallbackContent, 'nano-banana-flash-preview');
          } else {
            throw backupError;
          }
        }
