...
export type TabType = 'channels' | 'archetypes' | 'generate' | 'history' | 'translate' | 'api-docs' | 'batch' | 'admin';
...
const baseNavItems: { id: TabType; label: string; icon: React.ReactNode; adminOnly?: boolean }[] = [
...
const adminNavItems: { id: TabType; label: string; icon: React.ReactNode; adminOnly: boolean }[] = [
  { id: 'batch', label: 'Mass Generation', icon: <Layers size={18} />, adminOnly: true },
  { id: 'admin', label: 'Admin Panel', icon: <Settings size={18} />, adminOnly: true },
];
...
export default function Sidebar({ activeTab, onTabChange }: SidebarProps) {
  ...
  const handleTabChange = (tabId: TabType) => {
    // Handle special navigation for bulk/admin pages
    if (tabId === 'batch') {
      router.push('/bulk');
    } else if (tabId === 'admin') {
      router.push('/admin');
